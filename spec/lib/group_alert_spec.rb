...
RSpec.describe ::Kolide::GroupAlert do
  ...
  it "will match the corresponding user by IP address" do
    ...
    pm = group_pms.first
    expected_row = "| [#{device.uid}](https://k2.kolide.com/my/inventory/devices/#{device.uid}) | #{device.name} | #{device.primary_user_name} | #{device.hardware_model} | #{user.ip_address} (@#{user.username} [kolide-assign user=#{user.id} device=#{device.id}]) |"
    expect(pm.first_post.raw).to eq(I18n.t('kolide.group_alert.body', rows: expected_row).strip)
  end
